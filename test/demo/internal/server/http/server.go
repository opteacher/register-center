...
func registerService(e *bm.Engine, svc *service.Service) {
	pb.RegisterDemoBMServer(e, svc)
	// 将HTTP接口注册到Kong中
	if dsSvc, err := server.DiscoveryService(); err != nil {
		log.Error("Fetch discovery service error: %v", err)
	} else if data, err := utils.PickPathsFromSwaggerJSON("/Users/zhaojiachen/Projects/register-center/test/demo/api/api.swagger.json"); err != nil {
