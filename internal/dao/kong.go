...
func (k *Kong) DelService(name string) error {
	// TODO: 暂时只写了删除服务的逻辑，服务相关的upstreams、routes都还没处理
	if _, err := http.NewRequest(http.MethodDelete, k.GetAdminAddr(fmt.Sprintf("services/%s", name)), nil); err != nil {
		return err
	}
	return nil
}
